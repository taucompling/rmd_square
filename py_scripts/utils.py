...
def get_lexica_representations(type, lexica, only_prag):
    l = lexica[type-len(lexica)]
    for column in l.T:
        if -5 in column:
            l = np.delete(l, np.where(l == -5)[1][0], 1)
    return f"{check_literal_or_pragmatic(type, lexica, only_prag)}:\n{l}"


def check_literal_or_pragmatic(type, lexica, only_prag):
    if type > len(lexica)-1 or only_prag:
        return "pragmatic"
    return "literal"
...
def get_target_bins(type_idx, bins, level, number_of_lexica, puzzle):
    try:
        ...
        raise Exception("Competitor or Target Type is not a valid type. Change them in the yaml-file.")
    target_bins = []
    for l in level:
