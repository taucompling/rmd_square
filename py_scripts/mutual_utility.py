...
def get_utils(typeList, all_messages, all_states, lam,alpha,mutual_exclusivity, result_path, predefined, state_priors):
    ...
    if os.path.isfile("experiments/%s/matrices/umatrix-%s-s%s-m%s-lam%d-a%d-me%s.csv" %(result_path, str(state_priors.flatten()), str(states), str(messages),lam,alpha,str(mutual_exclusivity))) and not predefined:
    ...
    with open("experiments/%s/matrices/umatrix-%s-s%s-m%s-lam%d-a%d-me%s.csv" %(result_path, str(state_priors.flatten()), str(states), str(messages),lam,alpha,str(mutual_exclusivity)), "w") as file:
